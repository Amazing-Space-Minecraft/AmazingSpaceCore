...
object Bazaars : SLComponent() {
	...
	private fun openItemMenu(
		player: Player, terrId: Oid<Territory>, item: String, sort: SortingBy, descend: Boolean, remote: Boolean
	): Unit = Tasks.async {
	...
	private fun openPurchaseMenu(
		player: Player, item: BazaarItem, sellerName: String, currentAmount: Int, remote: Boolean
	) {
		MenuHelper.apply {
			...
			fun addButton(amount: Int) {
				...
				pane.addItem(guiButton(buttonItem) {
			...
			if (currentAmount == 0) {
				...
				pane.addItem(guiButton(Material.HOPPER) {
					playerClicker.closeInventory()
